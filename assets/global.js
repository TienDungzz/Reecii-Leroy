...
function logoReveal() {
  ...
  if (preloadScreen) setTimeout(() => { preloadScreen.classList.add("off--ready") }, 10);
}
...
class HeaderDrawer extends MenuDrawer {
  ...
  openMenuDrawer(summaryElement) {
    ...
    this.borderOffset =
      this.borderOffset || this.closest('.drawer--menu').classList.contains('header-wrapper--border-bottom') ? 1 : 0;
    document.documentElement.style.setProperty(
...
class RecentlyViewedProducts extends HTMLElement {
  ...
  load() {
    fetch(this.getUrl())
      .then(response => response.text())
      .then(text => {
        const recentlyViewed = new DOMParser().parseFromString(text, 'text/html').querySelector('recently-viewed-products');
  ...
  getUrl() {
    ...
    if (this.dataset.productId && listItems.includes(parseInt(this.dataset.productId))) listItems.splice(listItems.indexOf(parseInt(this.dataset.productId)), 1);
    return this.dataset.url + listItems.map((id) => 'id:' + id).slice(0, parseInt(this.dataset.limit)).join('%20OR%20');
  }
}
